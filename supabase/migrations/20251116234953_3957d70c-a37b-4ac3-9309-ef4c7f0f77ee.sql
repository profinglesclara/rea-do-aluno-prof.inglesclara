-- Atualizar função get_dashboard_aluno com os status corretos
...
BEGIN
  ...
  -- Contar atividades sugeridas pendentes (não concluídas)
  SELECT COUNT(*)
  INTO v_atividades_sugeridas_pendentes
  FROM public.atividades_sugeridas
  WHERE aluno = p_aluno AND status != 'Concluída'::status_sugestao;

  -- Contar atividades tarefas pendentes (disponíveis, não concluídas)
  SELECT COUNT(*)
  INTO v_atividades_tarefas_pendentes
  FROM public.atividades_tarefas
  WHERE aluno = p_aluno AND status = 'Disponível'::status_atividade;
